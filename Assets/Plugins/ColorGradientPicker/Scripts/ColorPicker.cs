...
public class ColorPicker : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // add all the scrap button onclicks:
        GameObject ScrapColors = gameObject.transform.Find("ScrapColors").gameObject;

        Button[] buttons = ScrapColors.GetComponentsInChildren<Button>();

        Array.ForEach(buttons, button => 
            button.onClick.AddListener( 
                delegate {
                    var color = button.gameObject.GetComponent<Image>().color;
    ...
    {   
        ...
        if(done)
        {
            ...
            instance.transform.GetChild(0).GetChild(0).GetComponent<TMP_Text>().text = message;
